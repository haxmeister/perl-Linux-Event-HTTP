...
plan skip_all => 'openssl command is required for native HTTP/2 high-level multiplex test'
    if !defined $openssl;
...
my $server = Linux::Event::HTTP::Server->new(
    ...
    port  => 0,
    http2 => 1,
    _http2_session_class => 'Linux::Event::HTTP::_HTTP2::Native',
    tls   => {
        cert_file => $cert,
        key_file  => $key,
    },
...
my $guard = Linux::Event::Kernel::Timer->new(
    ...
    after => 10,
    on_timer => sub ($timer) {
        die "high-level native HTTP/2 high-level multiplex test timed out\n";
    },
);
...
is_deeply($state->{errors}, [],
    'native-backed multiplexed high-level Client has no errors');
