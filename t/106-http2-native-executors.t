...
my $listener = Linux::Event::IO::Sock::Listener->new(
    ...
    stream => {
        on_ready => sub ($stream) {
            ...
            $server_executor{$stream->fd} = $executor;
        },
        on_data => sub ($stream, $bytes) {
            my $executor = $server_executor{$stream->fd}
                or die "native server executor is unavailable\n";
            my $consumed = $executor->input($bytes);
            die "native server executor left input unconsumed\n"
                if $consumed != length($bytes);
        },
        on_drain => sub ($stream) {
            my $executor = $server_executor{$stream->fd} or return;
        ...
        on_close => sub ($stream) {
            my $executor = delete $server_executor{$stream->fd};
...
my $client_stream = Linux::Event::IO::Sock::Stream->connect(
    ...
    on_data => sub ($stream, $bytes) {
        ...
        die "native client executor left input unconsumed\n"
            if $consumed != length($bytes);
    },
    on_drain => sub ($stream) {
