...
is_deeply(
    ...
    [ 'one', 'two' ],
    'header_values preserves duplicate values in wire order',
);
...
my $error = eval { $request->header_name(4); 1 };
ok(!$error, 'out-of-range header name index is rejected');
like($@, qr/header index out of range/, 'header index error is clear');
