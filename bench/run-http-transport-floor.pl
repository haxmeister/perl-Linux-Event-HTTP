...
my %case = (
    ...
    stream => {
        label => 'Linux::Event Stream floor',
...
say 'Linux::Event::Net::HTTP transport-floor comparison';
...
say 'http=full HTTP/1 request/response path';
say 'stream=native CRLFCRLF delimiter framer + prebuilt HTTP response';
say 'mode=single-process loopback-tcp shared-client';

my @names = qw(http stream);
for my $repeat (1 .. $repeats) {
    my @order = $repeat % 2 ? @names : reverse @names;
    for my $name (@order) {
...
if ($http && $stream && $http->{requests_per_second}) {
    printf "\nstream/http throughput ratio: %.3fx\n",
...
if (defined $json_path) {
    ...
    my $report = {
        benchmark => 'linux-event-net-http-transport-floor',
        ...
        contract => {
            http => 'Linux::Event::Net::HTTP server using the current production request/response API',
            stream => 'Linux::Event Sock::Stream using native Delimiter framing on CRLFCRLF and one prebuilt HTTP/1.1 200 response per message',
            excluded_from_stream => [qw(pico request_object response_object http_transaction_state http_response_semantics)],
        },
        summary => \@summary,
        records => \@records,
    };
...
sub start_server ($name, $port) {
    my $stdout_path = "/tmp/le-http-floor-$name-$$-out.log";
    my $stderr_path = "/tmp/le-http-floor-$name-$$-err.log";
    my $pid = fork();
    die "fork $name: $!\n" if !defined $pid;
...
sub usage ($exit) {
    print <<'USAGE';
Usage: perl -Mblib bench/run-http-transport-floor.pl [options]
...
The Stream floor is intentionally not an HTTP implementation. It recognizes
request boundaries with Linux::Event's native Delimiter framer on CRLFCRLF and
writes a prebuilt HTTP/1.1 200 response. This isolates generic Stream/transport
cost from Linux::Event::Net::HTTP request and response machinery.
USAGE
    exit $exit;
}
