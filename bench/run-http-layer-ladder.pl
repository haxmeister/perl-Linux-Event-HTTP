...
my %case = (
    stream => {
        label => '1 Stream native delimiter',
        command => [$^X, '-Mblib', "$Bin/servers/linuxevent-stream-floor.pl"],
    },
    perl_buffer => {
        label => '2 Stream + Perl buffer',
        command => [$^X, '-Mblib', "$Bin/servers/linuxevent-perl-buffer-floor.pl"],
    },
    parse => {
        label => '3 Stream + pico Request',
...
my @names = qw(stream perl_buffer parse http);
...
say 'Linux::Event::Net::HTTP layer ladder';
...
say '1=Stream native Delimiter(CRLFCRLF) + prebuilt response';
say '2=unframed Stream + Perl .=/index/substr buffering + prebuilt response';
say '3=same Perl buffer + pico parse/semantic validation/native Request construction + prebuilt response';
say '4=current full HTTP request/response lifecycle with native final-response experiment';
say 'mode=single-process loopback-tcp shared-client';
...
say '';
for my $i (0 .. $#summary - 1) {
    my ($from, $to) = @summary[$i, $i + 1];
    next if !$to->{requests_per_second};
    printf "%s / %s throughput: %.3fx\n",
        $from->{case}, $to->{case},
        $from->{requests_per_second} / $to->{requests_per_second};
}
my ($stream) = grep { $_->{case} eq 'stream' } @summary;
my ($http) = grep { $_->{case} eq 'http' } @summary;
if ($stream && $http && $http->{requests_per_second}) {
    printf "stream / full-http throughput: %.3fx\n",
        $stream->{requests_per_second} / $http->{requests_per_second};
}

if (defined $json_path) {
    my ($sysname, $nodename, $release, $version, $machine) = uname();
    my $report = {
        benchmark => 'linux-event-net-http-layer-ladder',
        ...
        contract => {
            stream => 'Sock::Stream native Delimiter framing on CRLFCRLF; one Perl on_message callback; prebuilt HTTP response',
            perl_buffer => 'unframed Sock::Stream; Perl input concatenation, CRLFCRLF index scan, destructive substr; prebuilt HTTP response',
            parse => 'same unframed Perl input buffer plus pico parse_request, HTTP semantic validation, native Request allocation/head copy, and Request wrapper; prebuilt HTTP response',
            http => 'current Linux::Event::Net::HTTP Connection request/response lifecycle with native default final-response experiment enabled',
            common => 'same raw client, request wire, prebuilt response payload size, persistent loopback TCP sockets, Stream read budget, and Stream write transport',
        },
        summary => \@summary,
        records => \@records,
    };
...
sub start_server ($name, $port) {
    my $stdout_path = "/tmp/le-http-ladder-$name-$$-out.log";
    my $stderr_path = "/tmp/le-http-ladder-$name-$$-err.log";
    my $pid = fork();
    die "fork $name: $!\n" if !defined $pid;
...
sub usage ($exit) {
    print <<'USAGE';
Usage: perl -Mblib bench/run-http-layer-ladder.pl [options]
...
The four stages intentionally add one HTTP-layer cost at a time while keeping
the raw client, request bytes, response bytes, Stream transport and loopback TCP
shape fixed. The two intermediate servers are benchmark-only diagnostics, not
